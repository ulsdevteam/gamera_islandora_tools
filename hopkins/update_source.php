...
function update_sources($hopkins_objects) {
  ...
  foreach ($hopkins_objects as $pid => $hopkins_object) {
    $new_source = $hopkins_object['mods_relatedItem_host_titleInfo_title_ms'] .
      (isset($hopkins_object['mods_originInfo_type_display_dateOther_s']) ? ' (' . $hopkins_object['mods_originInfo_type_display_dateOther_s'] . ')' : '');

    _log('PID: ' . $pid . ', old source = \'' . addslashes($hopkins_object['mods_relatedItem_host_titleInfo_title_ms']) . '\', '.
      'date: ' . $hopkins_object['mods_originInfo_type_display_dateOther_s'] . ', new source = \'' . $new_source . '\'');
/*    echo "<b>PID: </b> " . $pid ."<br>";
    echo "<b>Current Source:</b> " . $hopkins_object['mods_relatedItem_host_titleInfo_title_ms'] . "<br>";
    echo "Date: " . $hopkins_object['mods_originInfo_type_display_dateOther_s'] . "<br>";
    echo "<span style='color:green'><b>New Source: </b>" . $new_source . "</span><br>";
*/
    echo $new_source . "<br>";
...
echo "<pre>".htmlspecialchars($MODS_datastream->content) ."</pre><hr>";
...
echo $fly_transform . "<br><pre>" .
  htmlspecialchars($new_MODS) . "</pre><hr>";
...
{xsl:stylesheet version="1.0"
   ...
   {xsl:template match="/ | @* | node()"}
         ...
         {/xsl:copy}
   {/xsl:template}
   {xsl:template match="/mods:mods/mods:relatedItem[@type=\'host\']/mods:titleInfo/mods:title"}' . $new_source . '{/xsl:template}
{/xsl:stylesheet}');
